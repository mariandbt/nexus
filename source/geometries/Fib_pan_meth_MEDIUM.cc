...
REGISTER_CLASS(Fib_pan_meth_MEDIUM, GeometryBase)

namespace nexus {

  Fib_pan_meth_MEDIUM::Fib_pan_meth_MEDIUM():
    GeometryBase(),
    liquid_(false),
    ...
    coated_(true)
  {
    msg_ = new G4GenericMessenger(this, "/Geometry/Fib_pan_meth_MEDIUM/",
      "Control commands of geometry Fib_pan_meth_MEDIUM.");
  ...
  void Fib_pan_meth_MEDIUM::Construct()
  {

    G4cout << "[Fib_pan_meth_MEDIUM] *** Barrel Fiber prototype ***" << G4endl;
    G4cout << "[Fib_pan_meth_MEDIUM] Using " << fiber_type_ << " fibers"<< G4endl;
    ...
    // // the volume.
    // IonizationSD* ionizsd = new IonizationSD("/Fib_pan_meth_MEDIUM");
    ...
    G4cout << "[Fib_pan_meth_MEDIUM] Using " << n_fibers << " fibers per panel"<< G4endl;


    G4int n_sensors = 5; // number of sensors per panel
    G4double dl_sens = panel_width_/n_sensors; // distance between sensors

    G4cout << "[Fib_pan_meth_MEDIUM] Using " << n_panels*n_sensors << " sensors in total"<< G4endl;
    ...
    G4cout << "[Fib_pan_meth_MEDIUM] Using " << n_panels << " panels" << G4endl;
    ...
    if (methacrylate_)
    {
      cyl_vertex_gen_ = new CylinderPointSampler2020(0, hh - fiber_diameter_/2. - window_thickness_, (panel_length_ - 2*fiber_end_z)/2, 0, 2 * M_PI);
    } else if (methacrylate_ == false){
      cyl_vertex_gen_ = new CylinderPointSampler2020(0, hh - fiber_diameter_/2., (panel_length_ - 2*fiber_end_z)/2, 0, 2 * M_PI,
    ...
    new G4PVPlacement(0, G4ThreeVector(0, 0, (panel_length_ + teflon_thickness_)/2.),
    ...
    // G4cout << "[Fib_pan_meth_MEDIUM] Barrel with " << n_fibers << " fibers" << G4endl;
    ...
    h_Planck * c_light / (699.57 * nm), h_Planck * c_light / (630.00 * nm),
      ...
      G4cout << "[Fib_pan_meth_MEDIUM] Using " << t_binning << " [ns] binning"<< G4endl;
    ...
    // for (G4int itheta=0; itheta < 3; itheta++) {
      ...
      // for (G4int ii=0; ii < 1; ii++) {
          ...
          new G4PVPlacement(0, G4ThreeVector(xx_f, yy_f, z_p + panel_length_/2. - fiber_end_z - fiber_length_/2.),
                            fiber_logic, "FIBER-" + label + label2, world_logic_vol,
                            false, n_panels + ii, false);
          new G4PVPlacement(0, G4ThreeVector(xx_f, yy_f, z_p + (panel_length_ - fiber_end_z)/2.),
      ...
      // for (G4int jj=0; jj < 3; jj++) {
            ...
            new G4PVPlacement(sensor_rot, G4ThreeVector(xx_s, yy_s, z_p - (panel_length_ - sens_z)/2.),
  ...
  G4ThreeVector Fib_pan_meth_MEDIUM::GenerateVertex(const G4String& region) const
  {
