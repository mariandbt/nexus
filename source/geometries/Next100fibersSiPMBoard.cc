// -----------------------------------------------------------------------------
// nexus | Next100fibersSiPMBoard.cc
//
...
Next100fibersSiPMBoard::Next100fibersSiPMBoard():
  GeometryBase     (),
  size_            (122.40  * mm),
  pitch_           ( 15.55  * mm),
  margin_          (  6.775 * mm),
  ...
  mpv_             (nullptr),
  vtxgen_          (nullptr),
  sipm_            (new Next100fibersSiPM())
{
  msg_ = new G4GenericMessenger(this, "/Geometry/Next100fibers/",
                                "Control commands of the NEXT-100 geometry.");

  msg_->DeclareProperty("sipm_board_vis", visibility_, "Visibility of Next100fibersSiPMBoard.");

  msg_->DeclareProperty("sipm_vis", sipm_visibility_, "Visibility of Next100fibers SiPMs.");
...
void Next100fibersSiPMBoard::Construct()
{
  // Make sure the mother physical volume is actually valid
  if (!mpv_)
    G4Exception("[Next100fibersSiPMBoard]", "Construct()",
...
G4ThreeVector Next100fibersSiPMBoard::GenerateVertex(const G4String&) const
{
