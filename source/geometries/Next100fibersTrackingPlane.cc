// ----------------------------------------------------------------------------
// nexus | Next100fibersTrackingPlane.cc
//
...
#include "Next100fibersSiPMBoard.h"
...
Next100fibersTrackingPlane::Next100fibersTrackingPlane():
  GeometryBase(),
  ...
  visibility_(true),
  sipm_board_geom_(new Next100fibersSiPMBoard),
...
void Next100fibersTrackingPlane::Construct()
{
  // Make sure the pointer to the mother volume is actually defined
  if (!mpv_)
    G4Exception("[Next100fibersTrackingPlane]", "Construct()",
...
void Next100fibersTrackingPlane::PlaceSiPMBoardColumns(G4int num_boards,
...
void Next100fibersTrackingPlane::PrintSiPMPosInGas() const
{
...
void Next100fibersTrackingPlane::GetSiPMPosInGas(std::vector<G4ThreeVector>& sipm_pos) const
{
  auto sipm_positions = sipm_board_geom_->GetSiPMPositions();
  for (unsigned int i=0; i<board_pos_.size(); ++i) {
    for (unsigned int j=0; j<sipm_positions.size(); ++j) {
...
G4ThreeVector Next100fibersTrackingPlane::GenerateVertex(const G4String& region) const
{
...
G4double Next100fibersTrackingPlane::GetSiPMPitch() const {
