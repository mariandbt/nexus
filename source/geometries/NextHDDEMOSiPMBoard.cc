  ...
  sipm_            (new NextHDDEMOSiPM())
{
  ...
  msg_->DeclareProperty("sipm_board_vis", visibility_,
                        "Visibility of NextHDDEMOSiPMBoard.");

  msg_->DeclareProperty("sipm_vis", sipm_visibility_,
                        "Visibility of NextHDDEMO SiPMs.");
...
void NextHDDEMOSiPMBoard::Construct()
{
  // Make sure the mother physical volume is actually valid
  if (!mpv_)
    G4Exception("[NextHDDEMOSiPMBoard]", "Construct()",
...
G4ThreeVector NextHDDEMOSiPMBoard::GenerateVertex(const G4String&) const
{
