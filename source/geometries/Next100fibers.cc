...
#include "Next100fibers.h"
#include "LSCHallA.h"
#include "Next100fibersShielding.h"
#include "Next100fibersVessel.h"
#include "Next100fibersIcs.h"
...
namespace nexus {
  ...
  REGISTER_CLASS(Next100fibers, GeometryBase)

  using namespace CLHEP;

  Next100fibers::Next100fibers():
    GeometryBase(),
    // Lab dimensions
    lab_size_ (5. * m),
  ...
  // Shielding
  shielding_ = new Next100fibersShielding();

  //Lab walls
  hallA_walls_ = new LSCHallA();

  // Vessel
  vessel_ = new Next100fibersVessel(ics_ep_lip_width_);

  // Internal copper shielding
  ics_ = new Next100fibersIcs(ics_ep_lip_width_);

  // Inner Elements
  inner_elements_ = new Next100fibersInnerElements(grid_thickness_);
  ...
  G4ThreeVector Next100fibers::GenerateVertex(const G4String& region) const
  {
    ...
    // Lab walls
      ...
      if (!lab_walls_)
        G4Exception("[Next100fibers]", "GenerateVertex()", FatalException,
  ...
  G4ThreeVector Next100fibers::ProjectToRegion(const G4String& region,
					 ...
					 const G4ThreeVector& dir) const
  {
    ...
    if ((region == "HALLA_OUTER") || (region == "HALLA_INNER")){
      if (!lab_walls_)
	G4Exception("[Next100fibers]", "ProjectToRegion()", FatalException,
