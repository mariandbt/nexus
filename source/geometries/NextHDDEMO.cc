...
namespace nexus {
  ...
  REGISTER_CLASS(NextHDDEMO, GeometryBase)

  using namespace CLHEP;

  NextHDDEMO::NextHDDEMO():
    GeometryBase(),
    // Lab dimensions
    lab_size_ (5. * m),
    ...
    // note that if grid thickness change it must be also changed in NextHDDEMOFieldCage.cc
  ...
  // Shielding
  shielding_ = new NextHDDEMOShielding();

  //Lab walls
  hallA_walls_ = new LSCHallA();

  // Vessel
  vessel_ = new NextHDDEMOVessel();

  // Internal copper shielding
  ics_ = new NextHDDEMOIcs();

  // Inner Elements
  inner_elements_ = new NextHDDEMOInnerElements();
  ...
  void NextHDDEMO::Construct()
  {
    G4cout << "[NextHDDEMO] Constructor called" << G4endl;
  ...
  G4ThreeVector NextHDDEMO::GenerateVertex(const G4String& region) const
  {
    ...
    // Lab walls
      ...
      if (!lab_walls_)
        G4Exception("[NextHDDEMO]", "GenerateVertex()", FatalException,
